...
def cc2yolo_bbox(tlwh):  # [xmin,ymin,width,height] -> [x,y,width,height](归一化)
    ret = np.asarray(tlwh.cpu()).copy()
    for item in ret:
        item[0] = item[0] + item[2] / 2.0
        item[1] = item[1] + item[3] / 2.0
    return ret
...
def main():
    ...
    pose_estimator.cfg.visualizer.save_dir = '/root/Deep-OC-SORT/results/poses/'
    ...
    for (img, np_img), label, info, idx in loader:
        ...
        if pred is None:
            continue
        pred_pose = process_one_image(args, np_img[0].numpy(), cc2yolo_bbox(pred[:, :4]), pose_estimator, visualizer)
        # Nx5 of (x1, y1, x2, y2, ID)
        # targets = tracker.update(pred, img, np_img[0].numpy(), tag)
        targets = tracker.update(pred, pred_pose, img, np_img[0].numpy(), tag)
