...
def associate(
    ...
    pose_emb_cost = None
    if not emb_off:
        pose_emb_cost = None if (
                trk_pose_embs.shape[0] == 0 or pose_embs.shape[0] == 0) else pose_embs @ trk_pose_embs.T
    ...
    if min(iou_matrix.shape) > 0:
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            if not aw_off:
                w_matrix = compute_aw_new_metric(pose_emb_cost, w_assoc_emb, aw_param)
                pose_emb_cost *= w_matrix
            else:
                pose_emb_cost *= w_assoc_emb

            # 损失函数的构成
            final_cost = -(iou_matrix + angle_diff_cost + emb_cost + pose_emb_cost)
